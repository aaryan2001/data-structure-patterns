...
void testProblem1() {
    Matrix3x3 M(Vector3D(2.0f, 3.0f, 8.0f), Vector3D(6.0f, 0.0f, -3.0f),
    ...
    std::cout << "Has M a inverse? " << (M.hasInverse() ? "Yes" : "No")
        << std::endl;
...
//   std::cout << "Signed area of rotated polygon: "
//             << lRotatedPolygon.getSignedArea() << std::endl;

//   for (size_t i = 0; i < lRotatedPolygon.getNumberOfVertices(); i++) {
//     if ((std::round(
//              lPolygon.getVertex(i).dot(lRotatedPolygon.getVertex(i) * 10.0f)
//              / 10.0f) != 0.0f)) {
//       std::cout << "Rotation error of vertex " << lPolygon.getVertex(i)
//                 << std::endl;
